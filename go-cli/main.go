// Command WebsiteAnalysisApis2-cli is a CLI + REPL for the WebsiteAnalysisApis2 SDK,
...
const prompt = "website-analysis-apis2"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "dead_link_checker performance screenshot seo ssl tech_stack_detection"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      WEBSITE_ANALYSIS_APIS2_APIKEY (api key), WEBSITE_ANALYSIS_APIS2_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: WEBSITE_ANALYSIS_APIS2_APIKEY carries the API key and
	// WEBSITE_ANALYSIS_APIS2_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("WEBSITE_ANALYSIS_APIS2_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("WEBSITE_ANALYSIS_APIS2_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewWebsiteAnalysisApis2SDK(opts)
