# website-analysis-apis2-cli

boru-driven command-line client **and** interactive REPL for the WebsiteAnalysisApis2
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/website-analysis-apis2-cli)
make build

# 2. See usage (words, entities, env vars)
./website-analysis-apis2-cli --help

# 3. Provide credentials once, via the environment
export WEBSITE_ANALYSIS_APIS2_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./website-analysis-apis2-cli list dead_link_checker
./website-analysis-apis2-cli list performance

# 5. Override the API base URL for a single call
WEBSITE_ANALYSIS_APIS2_BASE=https://api.example.com ./website-analysis-apis2-cli list dead_link_checker

# 6. No arguments -> interactive REPL
./website-analysis-apis2-cli
website-analysis-apis2> list dead_link_checker
website-analysis-apis2> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/website-analysis-apis2-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/website-analysis-apis2-cli list dead_link_checker
   ```
...
Run with no arguments to open a REPL (prompt `website-analysis-apis2>`). Each line is
evaluated as its own boru expression:

```text
$ ./website-analysis-apis2-cli
website-analysis-apis2> list dead_link_checker
website-analysis-apis2> /help
website-analysis-apis2> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `dead_link_checker`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/website-analysis-apis2-cli`. |
...
(`./website-analysis-apis2-cli <expr>`) and interactively (the REPL), and expressions compose the
