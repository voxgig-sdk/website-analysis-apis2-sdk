...
class PerformanceEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PerformanceEntity(self._client, opts)
    ...
    def match_get(self) -> Performance:
    ...
    def load(self, reqmatch=None, ctrl=None) -> Performance:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Performance().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
