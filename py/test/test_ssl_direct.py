...
class TestSslDirect:
    ...
    def test_should_direct_load_ssl(self):
        setup = _ssl_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-ssl", "live" if setup["live"] else "unit")
        if _skip:
