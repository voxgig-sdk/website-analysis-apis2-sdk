...
class TestDeadLinkCheckerDirect:

    def test_should_direct_list_dead_link_checker(self):
        setup = _dead_link_checker_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-dead_link_checker", "live" if setup["live"] else "unit")
        if _skip:
...
def _dead_link_checker_direct_setup(mockres):
