...
class TestSeoDirect:
    ...
    def test_should_direct_list_seo(self):
        setup = _seo_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-seo", "live" if setup["live"] else "unit")
        if _skip:
