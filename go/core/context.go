...
func NewContext(ctxmap map[string]any, basectx *Context) *Context {
	...
	if c := getCtxProp(ctxmap, "client"); c != nil {
		if sdk, ok := c.(*WebsiteAnalysisApis2SDK); ok {
			ctx.Client = sdk
		}
	}
...
func (ctx *Context) MakeError(code string, msg string) *WebsiteAnalysisApis2Error {
	return NewWebsiteAnalysisApis2Error(code, msg, ctx)
}
