# WebsiteAnalysisApis2 Golang SDK Reference

Complete API reference for the WebsiteAnalysisApis2 Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *WebsiteAnalysisApis2SDK`
...
#### `DeadLinkChecker(data map[string]any) WebsiteAnalysisApis2Entity`

Create a new `DeadLinkChecker` entity instance. Pass `nil` for no initial data.

#### `Performance(data map[string]any) WebsiteAnalysisApis2Entity`

Create a new `Performance` entity instance. Pass `nil` for no initial data.

#### `Screenshot(data map[string]any) WebsiteAnalysisApis2Entity`

Create a new `Screenshot` entity instance. Pass `nil` for no initial data.

#### `Seo(data map[string]any) WebsiteAnalysisApis2Entity`

Create a new `Seo` entity instance. Pass `nil` for no initial data.

#### `Ssl(data map[string]any) WebsiteAnalysisApis2Entity`

Create a new `Ssl` entity instance. Pass `nil` for no initial data.

#### `TechStackDetection(data map[string]any) WebsiteAnalysisApis2Entity`
...
fmt.Println(deadLinkChecker.GetName()) // "dead_link_checker"
```
...
```go
results, err := client.DeadLinkChecker(nil).List(nil, nil)
if err != nil {
    panic(err)
}
fmt.Println(results)
...
fmt.Println(performance.GetName()) // "performance"
```
...
fmt.Println(screenshot.GetName()) // "screenshot"
```
...
```go
results, err := client.Seo(nil).List(nil, nil)
if err != nil {
    panic(err)
}
fmt.Println(results)
...
```go
techStackDetection := client.TechStackDetection(nil)
fmt.Println(techStackDetection.GetName()) // "tech_stack_detection"
```
...
```go
results, err := client.TechStackDetection(nil).List(nil, nil)
if err != nil {
    panic(err)
}
fmt.Println(results)
