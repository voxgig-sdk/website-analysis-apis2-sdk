...
// WebsiteAnalysisApis2 SDK feature test
//
...
class FtCtrl extends WebsiteAnalysisApis2Control
{
...
class FtClient
{
    ...
    private WebsiteAnalysisApis2Utility $utility;

    public function __construct(WebsiteAnalysisApis2Utility $utility, array $options)
    {
    ...
    public function get_utility(): WebsiteAnalysisApis2Utility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (WebsiteAnalysisApis2Context $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public WebsiteAnalysisApis2Context $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (WebsiteAnalysisApis2Context $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = WebsiteAnalysisApis2Config::make_feature($name);
    ...
    private static function buildUrl(WebsiteAnalysisApis2Spec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof WebsiteAnalysisApis2Error) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new WebsiteAnalysisApis2Response($fetched);
        }
    ...
    private function fail(WebsiteAnalysisApis2Context $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (WebsiteAnalysisApis2Context $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
