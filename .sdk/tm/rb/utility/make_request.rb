...
module WebsiteAnalysisApis2Utilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = WebsiteAnalysisApis2Response.new({})
    ...
    if fetch_err
      ...
      response = WebsiteAnalysisApis2Response.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = WebsiteAnalysisApis2Response.new(fetched)
    else
