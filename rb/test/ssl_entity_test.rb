...
class SslEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set WEBSITEANALYSISAPIS__TEST_SSL_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    ssl_ref01_match_dt0 = {}
    ssl_ref01_data_dt0_loaded = ssl_ref01_ent.load(ssl_ref01_match_dt0, nil)
    assert !ssl_ref01_data_dt0_loaded.nil?
...
def ssl_basic_setup(extra)
  ...
  entid_env_raw = ENV["WEBSITEANALYSISAPIS__TEST_SSL_ENTID"]
