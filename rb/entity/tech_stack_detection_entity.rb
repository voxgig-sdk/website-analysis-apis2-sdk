...
class TechStackDetectionEntity
  ...
  def make
    opts = @_entopts.dup
    TechStackDetectionEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = WebsiteAnalysisApis2Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [TechStackDetection, Hash] the current TechStackDetection data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = WebsiteAnalysisApis2Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of TechStackDetection fields)
  def match_get
  ...
  #
  # @param reqmatch [TechStackDetectionListMatch, Hash, nil] match filter (any subset of
  #   TechStackDetection fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<TechStackDetection>, Array] the matching TechStackDetection items; raises WebsiteAnalysisApis2Error on failure
