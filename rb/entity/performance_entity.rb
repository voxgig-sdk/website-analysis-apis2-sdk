...
class PerformanceEntity
  ...
  def make
    opts = @_entopts.dup
    PerformanceEntity.new(@_client, opts)
  end
  ...
  # @return [Performance, Hash] the current Performance data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Performance fields)
  def match_get
  ...
  #
  # @param reqmatch [PerformanceLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Performance.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Performance, Hash] the loaded Performance; raises WebsiteAnalysisApis2Error on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = WebsiteAnalysisApis2Helpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
