...
class ScreenshotEntity
  ...
  def make
    opts = @_entopts.dup
    ScreenshotEntity.new(@_client, opts)
  end
  ...
  # @return [Screenshot, Hash] the current Screenshot data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Screenshot fields)
  def match_get
  ...
  #
  # @param reqmatch [ScreenshotLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Screenshot.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Screenshot, Hash] the loaded Screenshot; raises WebsiteAnalysisApis2Error on failure
