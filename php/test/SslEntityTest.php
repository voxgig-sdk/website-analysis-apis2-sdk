...
class SslEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set WEBSITEANALYSISAPIS__TEST_SSL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $ssl_ref01_match_dt0 = [];
        $ssl_ref01_data_dt0_loaded = $ssl_ref01_ent->load($ssl_ref01_match_dt0, null);
        $this->assertNotNull($ssl_ref01_data_dt0_loaded);
...
function ssl_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("WEBSITEANALYSISAPIS__TEST_SSL_ENTID");
    $idmap_overridden = $entid_env_raw !== false && str_starts_with(trim($entid_env_raw), "{");

    $env = Runner::env_override([
        "WEBSITEANALYSISAPIS__TEST_SSL_ENTID" => $idmap,
        "WEBSITEANALYSISAPIS__TEST_LIVE" => "FALSE",
        "WEBSITEANALYSISAPIS__TEST_EXPLAIN" => "FALSE",
    ]);

    $idmap_resolved = Helpers::to_map(
        $env["WEBSITEANALYSISAPIS__TEST_SSL_ENTID"]);
