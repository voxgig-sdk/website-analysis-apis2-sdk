...
class PerformanceEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set WEBSITE_ANALYSIS_APIS2_TEST_PERFORMANCE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function performance_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("WEBSITE_ANALYSIS_APIS2_TEST_PERFORMANCE_ENTID");
