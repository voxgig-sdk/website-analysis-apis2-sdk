...
// WebsiteAnalysisApis2 SDK TechStackDetection entity
...
class TechStackDetectionEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "tech_stack_detection";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new TechStackDetectionEntity($this->_client, $opts);
    }

    /**
     * @param TechStackDetection|array $args TechStackDetection data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = WebsiteAnalysisApis2Helpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return TechStackDetection|array The current TechStackDetection data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of TechStackDetection fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = WebsiteAnalysisApis2Helpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of TechStackDetection fields).
     */
    ...
    /**
     * List TechStackDetection items matching the given filter.
     *
     * @param TechStackDetectionListMatch|array|null $reqmatch Match filter (any subset
     *   of TechStackDetection fields) as an assoc-array; TechStackDetectionListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return TechStackDetection[]|array A list of TechStackDetection items as assoc-arrays at
     *   the SDK boundary; throws WebsiteAnalysisApis2Error on failure (item-5 convention).
     */
