...
// WebsiteAnalysisApis2 SDK Performance entity
...
class PerformanceEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new PerformanceEntity($this->_client, $opts);
    }

    /**
     * @param Performance|array $args Performance data (assoc-array) to store.
     */
    ...
    /**
     * @return Performance|array The current Performance data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Performance fields).
     */
    ...
    /**
     * @return array The current match filter (any subset of Performance fields).
     */
    ...
    /**
     * Load a single Performance.
     *
     * @param PerformanceLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed PerformanceLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Performance|array The loaded Performance as an assoc-array at the
     *   SDK boundary; throws WebsiteAnalysisApis2Error on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = WebsiteAnalysisApis2Helpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
