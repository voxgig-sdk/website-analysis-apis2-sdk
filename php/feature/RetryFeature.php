...
// WebsiteAnalysisApis2 SDK retry feature
...
class WebsiteAnalysisApis2RetryFeature extends WebsiteAnalysisApis2BaseFeature
{
    ...
    public function init(WebsiteAnalysisApis2Context $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (WebsiteAnalysisApis2Context $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(WebsiteAnalysisApis2Context $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(WebsiteAnalysisApis2Context $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
