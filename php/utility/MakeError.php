...
class WebsiteAnalysisApis2MakeError
{
    public static function call(?WebsiteAnalysisApis2Context $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new WebsiteAnalysisApis2Context([], null);
        }
        $op = $ctx->op ?? new WebsiteAnalysisApis2Operation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new WebsiteAnalysisApis2Result([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof WebsiteAnalysisApis2Error) ? $err->msg : (string)$err;
        ...
        $sdk_err = new WebsiteAnalysisApis2Error('', $msg, $ctx);
        ...
        if ($err instanceof WebsiteAnalysisApis2Error) {
