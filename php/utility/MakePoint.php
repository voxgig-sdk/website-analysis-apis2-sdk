...
// WebsiteAnalysisApis2 SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class WebsiteAnalysisApis2MakePoint
{
    public static function call(WebsiteAnalysisApis2Context $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof WebsiteAnalysisApis2Error) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = WebsiteAnalysisApis2Helpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = WebsiteAnalysisApis2Helpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
