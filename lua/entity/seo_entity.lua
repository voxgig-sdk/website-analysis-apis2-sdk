...
local SeoEntity = {}
SeoEntity.__index = SeoEntity


function SeoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SeoEntity)
  self._name = "seo"
  self._client = client
...
function SeoEntity:get_name()
  return self._name
end


function SeoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SeoEntity.new(self._client, opts)
end
...
function SeoEntity:mark_deleted()
  self._deleted = true
end


function SeoEntity:deleted()
  return true == self._deleted
end


function SeoEntity:data_set(args)
  if args ~= nil then
...
function SeoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SeoEntity:match_set(args)
  if args ~= nil then
...
function SeoEntity:stream(action, args, callopts)
...
---@param reqmatch SeoListMatch
---@param ctrl? table
---@return Seo[]
---@return string? err
function SeoEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function SeoEntity:_run_op(ctx, post_done)
  local utility = self._utility
