...
local ScreenshotEntity = {}
ScreenshotEntity.__index = ScreenshotEntity


function ScreenshotEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ScreenshotEntity)
...
function ScreenshotEntity:get_name()
  return self._name
end


function ScreenshotEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ScreenshotEntity.new(self._client, opts)
end
...
function ScreenshotEntity:mark_deleted()
  self._deleted = true
end


function ScreenshotEntity:deleted()
  return true == self._deleted
end


function ScreenshotEntity:data_set(args)
  if args ~= nil then
...
function ScreenshotEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ScreenshotEntity:match_set(args)
  if args ~= nil then
...
function ScreenshotEntity:match_get()
...
function ScreenshotEntity:stream(action, args, callopts)
...
---@param reqmatch ScreenshotLoadMatch
---@param ctrl? table
---@return Screenshot
---@return string? err
function ScreenshotEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ScreenshotEntity:_run_op(ctx, post_done)
  local utility = self._utility
