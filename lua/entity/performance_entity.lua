...
local PerformanceEntity = {}
PerformanceEntity.__index = PerformanceEntity


function PerformanceEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PerformanceEntity)
...
function PerformanceEntity:get_name()
  return self._name
end


function PerformanceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PerformanceEntity.new(self._client, opts)
end
...
function PerformanceEntity:mark_deleted()
  self._deleted = true
end


function PerformanceEntity:deleted()
  return true == self._deleted
end


function PerformanceEntity:data_set(args)
  if args ~= nil then
...
function PerformanceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PerformanceEntity:match_set(args)
  if args ~= nil then
...
function PerformanceEntity:match_get()
...
function PerformanceEntity:stream(action, args, callopts)
...
---@param reqmatch PerformanceLoadMatch
---@param ctrl? table
---@return Performance
---@return string? err
function PerformanceEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function PerformanceEntity:_run_op(ctx, post_done)
  local utility = self._utility
