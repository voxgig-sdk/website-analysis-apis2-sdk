...
local function make_error_util(ctx, err)
  ...
  local msg = "WebsiteAnalysisApis2SDK: " .. opname .. ": " .. errmsg
  ...
  local sdk_err = WebsiteAnalysisApis2Error.new("", msg, ctx)
  sdk_err.result = ctx.utility.clean(ctx, result)
  sdk_err.spec = ctx.utility.clean(ctx, spec)

  if type(err) == "table" and getmetatable(err) == WebsiteAnalysisApis2Error then
