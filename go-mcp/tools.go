...
type Args struct {
	Entity string         `json:"entity" jsonschema:"dead_link_checker | performance | screenshot | seo | ssl | tech_stack_detection"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.WebsiteAnalysisApis2SDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "website-analysis-apis2_list",
		Description: "List records from WebsiteAnalysisApis2. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "website-analysis-apis2_load",
		Description: "Load a single record from WebsiteAnalysisApis2. " +
...
func runOp(client *sdk.WebsiteAnalysisApis2SDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.WebsiteAnalysisApis2SDK, name string) (sdk.WebsiteAnalysisApis2Entity, error) {
	switch strings.ToLower(name) {
	case "dead_link_checker":
		return client.DeadLinkChecker(nil), nil
	case "performance":
		return client.Performance(nil), nil
	case "screenshot":
		return client.Screenshot(nil), nil
	case "seo":
		return client.Seo(nil), nil
	case "ssl":
		return client.Ssl(nil), nil
	case "tech_stack_detection":
		return client.TechStackDetection(nil), nil
