...
class WebsiteAnalysisApis2SDK {
  ...
  async direct(fetchargs?: any) {
    if (!this._options.allow.op.includes('direct')) {
      return {
        ok: false,
        err: new Error('WebsiteAnalysisApis2SDK: direct: operation not allowed by' +
  ...
  async graphql(query: string, variables?: any, ctrl?: any) {
    ...
    if (!options.allow.op.includes('graphql')) {
      return {
        ok: false,
        err: new Error('WebsiteAnalysisApis2SDK: graphql: operation not allowed by' +
    ...
    if (null != errors && Array.isArray(errors) && 0 < errors.length) {
      ...
      const err: any = new Error('WebsiteAnalysisApis2SDK: graphql: ' +
  ...
  DeadLinkChecker(entopts?: Record<string, any>) {
    const self = this
    return new DeadLinkCheckerEntity(self, entopts)
  }
  ...
  // The argument is the entity OPTIONS object (passed to the entity
  // constructor as entopts), not initial entity data.
  Screenshot(entopts?: Record<string, any>) {
    const self = this
    return new ScreenshotEntity(self, entopts)
  }
  ...
  // The argument is the entity OPTIONS object (passed to the entity
  // constructor as entopts), not initial entity data.
  Seo(entopts?: Record<string, any>) {
    const self = this
    return new SeoEntity(self, entopts)
  }
  ...
  // The argument is the entity OPTIONS object (passed to the entity
  // constructor as entopts), not initial entity data.
  TechStackDetection(entopts?: Record<string, any>) {
    const self = this
    return new TechStackDetectionEntity(self, entopts)
  }
