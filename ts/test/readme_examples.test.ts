...
const SDK_NAME = 'WebsiteAnalysisApis2SDK'
...
const TEST_SEED = {"entity":{"dead_link_checker":{"test01":{"id":"test01"}},"performance":{"test01":{"id":"test01"}},"screenshot":{"test01":{"id":"test01"}},"seo":{"test01":{"id":"test01"}},"ssl":{"test01":{"id":"test01"}},"tech_stack_detection":{"test01":{"id":"test01"}}}}
...
function isRunnable(code: string): boolean {
  return (
    /new\s+WebsiteAnalysisApis2SDK\b/.test(code) ||
    /\bWebsiteAnalysisApis2SDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+WebsiteAnalysisApis2SDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/WebsiteAnalysisApis2SDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
