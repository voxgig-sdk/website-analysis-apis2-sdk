...
describe('SslEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set WEBSITE_ANALYSIS_APIS__TEST_SSL_ENTID JSON to run live')
      return
    }
    ...
    const ssl_ref01_data_dt0 = await ssl_ref01_ent.load(ssl_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['WEBSITE_ANALYSIS_APIS__TEST_SSL_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'WEBSITE_ANALYSIS_APIS__TEST_SSL_ENTID': idmap,
    'WEBSITE_ANALYSIS_APIS__TEST_LIVE': 'FALSE',
    'WEBSITE_ANALYSIS_APIS__TEST_EXPLAIN': 'FALSE',
  })

  idmap = env['WEBSITE_ANALYSIS_APIS__TEST_SSL_ENTID']
