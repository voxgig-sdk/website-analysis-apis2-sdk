...
describe('TechStackDetectionEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set WEBSITE_ANALYSIS_APIS__TEST_TECH_STACK_DETECTION_ENTID JSON to run live')
      return
    }
    ...
    const tech_stack_detection_ref01_list = await tech_stack_detection_ref01_ent.list(tech_stack_detection_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['WEBSITE_ANALYSIS_APIS__TEST_TECH_STACK_DETECTION_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'WEBSITE_ANALYSIS_APIS__TEST_TECH_STACK_DETECTION_ENTID': idmap,
    'WEBSITE_ANALYSIS_APIS__TEST_LIVE': 'FALSE',
    'WEBSITE_ANALYSIS_APIS__TEST_EXPLAIN': 'FALSE',
  })

  idmap = env['WEBSITE_ANALYSIS_APIS__TEST_TECH_STACK_DETECTION_ENTID']
