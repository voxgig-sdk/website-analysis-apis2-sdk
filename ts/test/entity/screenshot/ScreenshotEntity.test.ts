...
describe('ScreenshotEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'screenshot.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set WEBSITE_ANALYSIS_APIS__TEST_SCREENSHOT_ENTID JSON to run live')
      return
    }
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['WEBSITE_ANALYSIS_APIS__TEST_SCREENSHOT_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'WEBSITE_ANALYSIS_APIS__TEST_SCREENSHOT_ENTID': idmap,
    'WEBSITE_ANALYSIS_APIS__TEST_LIVE': 'FALSE',
    'WEBSITE_ANALYSIS_APIS__TEST_EXPLAIN': 'FALSE',
  })

  idmap = env['WEBSITE_ANALYSIS_APIS__TEST_SCREENSHOT_ENTID']
