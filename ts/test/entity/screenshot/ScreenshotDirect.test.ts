...
describe('ScreenshotDirect', async () => {
  ...
  test('direct-load-screenshot', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-screenshot', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['WEBSITE_ANALYSIS_APIS2_TEST_SCREENSHOT_ENTID']
